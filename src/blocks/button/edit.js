...
export default function edit({ setAttributes, attributes, isSelected, onReplace, mergeBlocks }) {
    const {
        style,
        align,
        width,
    ...
    return(
        <>
                ...
                <PanelBody
                    ...
                    <SelectControl
                        ...
                        options = {[
                            {
                                value: "one",
                                label: __("One", "inxperts-gutenberg-blocks"),
                            },
                            {
                                value: "two",
                                label: __("Two", "inxperts-gutenberg-blocks"),
                            },
                        ]}
                        value={style}
                        onChange={ ( value ) => { 
                ...
                <PanelBody
                    ...
                    >
                        <ToggleGroupControl label="Width" className="ix-togglegroup"
                            ...
                            <ToggleGroupControlOption value="inline" label="Inline" showTooltip={ true } aria-label="Inline" />
                            <ToggleGroupControlOption value="one" label="1/4" showTooltip={ true } aria-label="1/4" />
                            <ToggleGroupControlOption value="two" label="2/4" showTooltip={ true } aria-label="2/4" />
                            <ToggleGroupControlOption value="three" label="3/4" showTooltip={ true } aria-label="3/4" />
                            <ToggleGroupControlOption value="four" label="4/4" showTooltip={ true } aria-label="4/4" />
                        </ToggleGroupControl>
                </PanelBody>
            </InspectorControls>
            ...
            <div
				{ ...useBlockProps({className: `button--align-${align}` }) }
			>
                <RichText
                    ref={ richTextRef }
                    ...
                    className={ classnames(
                        `button button--style-${style} button--align-${align} button--width-${width}`
                    ) }
                    allowedFormats={ [ '' ] }
                    onReplace={ onReplace }
                    onMerge={ mergeBlocks }
